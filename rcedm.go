...
func fetchPageDatarcedm(url string, payload map[string]string, headers map[string]string) (map[string]interface{}, error) {
	client := &http.Client{}
	req, err := http.NewRequest("POST", url, strings.NewReader(mapToURLValuesrcedm(payload)))
	if err != nil {
		return nil, err
	}
...
func rcedm() {
	url := "https://buildout.com/plugins/2aca91d1f6cf41b86c50c7c313f3775a38e29ff9/inventory"
	...
	for {
		...
		data, err := fetchPageDatarcedm(url, payload, headers)
		if err != nil {
		...
		for _, entry := range inventory {
			...
			if url != "" {
				...
				sendDataToDatastorercedm(scraperData)
			}
		}
...
func mapToURLValuesrcedm(m map[string]string) string {
	values := []string{}
	for k, v := range m {
...
func sendDataToDatastorercedm(data Scraper) {
	...
	resp, err := http.Post("http://localhost:3000/add", "application/json", strings.NewReader(string(jsonData)))
	if err != nil {
